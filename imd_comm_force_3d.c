...
//MYMOD
#ifdef TTM
    NUMNEIGHS(to,i)   =NUMNEIGHS(from,i);
    to->fdi[i]=from->fdi[i];
    to->fdj[i]=from->fdj[i];
    to->fdk[i]=from->fdk[i];
#endif
...
#ifdef LOD
    LODP(to,i)   =LODP(from,i);
#endif


#ifdef NRB
   NRBI(to,i,0)  =NRBI(from,i,0);
   NRBI(to,i,1)  =NRBI(from,i,1);
   NRBI(to,i,2)  =NRBI(from,i,2);
   NRBI(to,i,3)  =NRBI(from,i,3);
   NRBI(to,i,4)  =NRBI(from,i,4);
   NRBI(to,i,5)  =NRBI(from,i,5);
   NRBI(to,i,6)  =NRBI(from,i,6);
   NRBI(to,i,7)  =NRBI(from,i,7);
   NRBI(to,i,8)  =NRBI(from,i,8);
   NRBI(to,i,9)  =NRBI(from,i,9);
   NRBI(to,i,10)  =NRBI(from,i,10);
   NRBI(to,i,11)  =NRBI(from,i,11);
...
//MYMOD
#ifdef TTM
    b->data[ j++]  = (real) NUMNEIGHS(from,i);
    b->data[j++]= (real) from->fdi[i]; //global fd-cell indices for advection step
    b->data[j++]= (real) from->fdj[i];
    b->data[j++]= (real) from->fdk[i];
#endif

#ifdef LOD
    b->data[ j++]  = LODP(from,i);
#endif
#ifdef NRB
    b->data[ j++]  = (real) NRBI(from,i,0);
    b->data[ j++]  = (real) NRBI(from,i,1);
    b->data[ j++]  = (real) NRBI(from,i,2);
    b->data[ j++]  = (real) NRBI(from,i,3);
    b->data[ j++]  = (real) NRBI(from,i,4);
    b->data[ j++]  = (real) NRBI(from,i,5);
    b->data[ j++]  = (real) NRBI(from,i,6);
    b->data[ j++]  = (real) NRBI(from,i,7);
    b->data[ j++]  = (real) NRBI(from,i,8);
    b->data[ j++]  = (real) NRBI(from,i,9);
    b->data[ j++]  = (real) NRBI(from,i,10);
    b->data[ j++]  = (real) NRBI(from,i,11);
    b->data[j++]   = (real) NRBBND(from,i);
    b->data[j++]   = (real) NRBNEIGH(from,i);

    b->data[j++]=REF_POS(from,i,X);  //ja, ind imd_nrb.c wird ref_pos zur kraftberechnung gebraucht
...
//ENDOF MYMOD
  ...
  b->n = j;
  if (b->n_max < b->n){
 printf("\n\n\n\n b->n_max:%d, b->n:%d,from->n:%d\n\n\n\n\n",b->n_max,b->n,from->n);

    error("Buffer overflow in pack_cell - increase msgbuf_size...(maybe atoms accumulated on proc 0 in a single md-cell?)");
  }
}
...
//MYMOD
#ifdef TTM
    NUMNEIGHS(to,i)  = b->data[ j++];
    to->fdi[i]=b->data[j++]; //global fd-cell indices for advection step
    to->fdj[i]=b->data[j++];
    to->fdk[i]=b->data[j++];
#endif
#ifdef LOD
    ...
    IMPULS(to,i,X)=b->data[j++];
    IMPULS(to,i,Y)=b->data[j++];
    IMPULS(to,i,Z)=b->data[j++];
#endif
...
*
*  WOZU soll das gut sein (ausser bei forces-covalent) ???
*
******************************************************************************/
...
*
*  WOZU soll das gut sein (ausser bei forces-covalent) ???
*  wird sonst nirgends aufgerufen
******************************************************************************/
