...
int nyeDone;
  ...
  nyeDone = 0;
    ...
    if ( (ensemble==ENS_MIK) || (ensemble==ENS_GLOK) || (ensemble==ENS_CG) )
    {
        if(lindef_int >0)
        {
...
#endif
            {
                ...
                have_valid_nbl = 0;
#endif
                //printf("proc:%d,steps:%d, main loop starts fix_cells\n",myid,steps);
                fix_cells();  
                //printf("proc:%d,steps:%d, main loop done fix_cells\n",myid,steps);
    ...
    //printf("proc:%d,steps:%d, main loop starts fix_cells\n",myid,steps);
    fix_cells();  
    //printf("proc:%d,steps:%d, main loop done fix cells\n",myid,steps);
...
#ifdef USEFCS
#ifdef PAIR
      //printf("proc:%d,steps:%d, main loop starts calc_forces\n",myid,steps);
      calc_forces(steps);
      //printf("proc:%d,steps:%d, main loop done calc_forces\n",myid,steps);

#endif
      calc_forces_fcs(steps);
#else
      //printf("proc:%d,steps:%d, main loop starts calc_forcesd\n",myid,steps);
      calc_forces(steps);
      //printf("proc:%d,steps:%d, main loop done calc_forces\n",myid,steps);
...
/* #ifdef BBOOST
      ...
      else
	{
	  ...
	  //printf("proc:%d,steps:%d, main loop starts stress tens\n",myid,steps);
	  ...
	  if ( (press_int > 0) && (steps >= (avpos_npwrites+1)*press_int - avpos_res*avpos_steps))
	      ...
	      if (tmpsteps == 0)
		{
		  ...
		  //	  printf("writing out: %d\n",avpos_npwrites);fflush(stdout); 
            ...
            //printf("proc:%d,steps:%d, main loop done stress tens\n",myid,steps);
...
//ENDOF
    if ((checkpt_int > 0) && (0 == steps % checkpt_int) && steps>0) 
    {
//MYMOD: BRAUCHE NICHT chkpt 0!!!!
       write_config( steps/checkpt_int, steps);

//MYMOD: NRB restart-file
#ifdef NRB
     nrb_writerestart(steps/checkpt_int); 
#endif
    }
//ENDOF MYMOD
    if ((eng_int  > 0) && (0 == steps % eng_int )) 
    {
        //printf("proc:%d,steps:%d, main loop starts write_eng\n",myid,steps);
	write_eng_file(steps);
        //printf("proc:%d,steps:%d, main loop done write eng\n",myid,steps);

    }
    if ((dist_int > 0) && (0 == steps % dist_int))
    {
       //printf("proc:%d,steps:%d, main loop starts dist\n",myid,steps);
       write_distrib(steps);
       //printf("proc:%d,steps:%d, main loop done dist\n",myid,steps);
    }
...
//ENDOF MYMOD
    ...
    if ((press_int > 0) && (0 == steps % press_int)) {
      //printf("proc:%d,steps:%d, main loop starts do_press_calc and writeconfig\n",myid,steps);

      if (!do_press_calc) error("pressure tensor incomplete for writing .press file");
       write_config_select( steps/press_int, "press",
                            write_atoms_press, write_header_press);
      //printf("proc:%d,steps:%d, main loop done do_press_calc\n",myid,steps);
    ...
    check_nblist();
#else
    //printf("proc:%d,steps:%d, main loop starts fix_cells 815\n",myid,steps);
    fix_cells();  
    //printf("proc:%d,steps:%d, main loop done fix_cells 817\n",myid,steps);
