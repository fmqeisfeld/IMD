...
//MYMOD ACHTUNG: Das urspruengliche Schema soll so wenig wie möglich verändert werden. Es wird nur dahingehend erweitert, dass
//		 die Atome direkt beim erstellen einen Versatz bekommen.
//
//		 box.x muss teilbar sein durch lattice const und boxparam.x (=box.x/a) muss wiederum teilbar sein duch cpu_dim.x
//		 um dies zu erreichen addiere ich zu box.x einen Zahl, d.h. effektiv wird shifx_rear groesser bzw. es entsteht ein 
//		 shift am probenende auch wenn vorher keiner da war
//		 Zunächst wird die komplette Box mit EZ gefuellt.
//		 Anschließend wird in der Schleife in generate_fcc geprueft ob die Atompositionen > shiftx_front bzw. kleiner als
//		 box_x.x-shiftx_rear sind. Nur dann wird das Atom platziert

  box_x.x+=shiftx_front+shiftx_rear;
  real box_x_old=box_x.x;
/*
  int ifudge=ceil(box_x.x/((double) cpu_dim.x * box_unit));
  box_x.x=(double) ifudge * ((double) cpu_dim.x) * box_unit; //box_x.x wird teilbar durch cpudim*lattice const
  box_param.x=(int) (box_x.x/box_unit);  
  shiftx_rear+=(box_x.x-box_x_old);
*/
  double boxpard=box_x.x/box_unit;
  int foo=ceil(boxpard/((double)cpu_dim.x));
  box_param.x=(int) foo*cpu_dim.x;
  box_x.x=((double) box_param.x)*box_unit;
  shiftx_rear+=(box_x.x-box_x_old);

  //selbes fuer y richtung
  box_y.y+=shifty_front+shifty_rear;
  real box_y_old=box_y.y;

  boxpard=box_y.y/box_unit;
  foo=ceil(boxpard/((double)cpu_dim.y));
  box_param.y=(int) foo*cpu_dim.y;
  box_y.y=((double) box_param.y)*box_unit;
  shifty_rear+=(box_y.y-box_y_old);
...
printf("*************************************************\n");
printf("*                  GENERATE                     *\n");
printf("* shiftx_front:%.2f\n",shiftx_front);
printf("* shiftx_rear:%.2f\n",shiftx_rear);
printf("* shifty_front:%.2f\n",shifty_front);
printf("* shifty_rear:%.2f\n",shifty_rear);
printf("* shiftz_front:%.2f\n",shiftz_front);
printf("* shiftz_rear:%.2f\n",shiftz_rear);
printf("* box_x.x:%.4e\n* box_param:%d\n* boxpar/cpu:%f\n* boxx/a:%.4e\n* boxx/a/cpu:%.4e\n* shiftx_rear_adjusted:%.4e\n",
          box_x.x,     box_param.x,
          (double) box_param.x/cpu_dim.x,
          box_x.x/box_unit,
          box_x.x/((double) cpu_dim.x * box_unit),
          shiftx_rear);

printf("* box_y.y:%.4e\n* box_param:%d\n* boxpar/cpu:%f\n* boxy/a:%.4e\n* boxy/a/cpu:%.4e\n* shifty_rear_adjusted:%.4e\n",
          box_y.y,     box_param.y,
          (double) box_param.y/cpu_dim.y,
          box_y.y/box_unit,
          box_y.y/((double) cpu_dim.y * box_unit),
          shifty_rear);
printf("*************************************************\n");
}
  box_z.z+=shiftz_front+shiftz_rear;
//ENDOF MYMOD
  make_box();
}
...
void generate_fcc(int maxtyp)
{
  ...
  if (myid==0)
    if ((box_param.x % cpu_dim.x) || (box_param.y % cpu_dim.y) || 
        (box_param.z % cpu_dim.z))
  {  
      //MYMOD: More detailed error msg
      char errbuf[300];
      sprintf(errbuf,"box_param must be commensurate with cpu_dim. boxparx/cpux:%f,boxpary/cpuy:%f,boxpaz/cpuz:%f\n",
		     (real) box_param.x/cpu_dim.x, (real) box_param.y/cpu_dim.y, (real) box_param.z/cpu_dim.z);
      error("box_param must be commensurate with cpu_dim");
  }
#endif
...
//MYMOD
/*
printf("CPU:%d,min.x:%d,max.x:%d,myx:%d,box.x:%.6e,boxunit:%.6e,minx2:%.16e,max2:%.16e\n",myid,min.x,max.x,my_coord.x,box_x.x,box_unit,
	(min.x + 0.5) * bu,(max.x+0.5)*bu);
*/
//ENDOF MYMOD
...
if(xx<shiftx_front)
  continue;
if(xx>box_x.x-shiftx_rear)
  continue;

if(yy<shifty_front) continue;
if(yy>box_y.y-shifty_rear) continue;
//ENDOF MYMOD
...
//MYMOD
	  ...
	  char errbuf[255];
int cellx=(int)(global_cell_dim.x * (xx*tbox_x.x + yy*tbox_x.y + zz*tbox_x.z));
	  sprintf(errbuf,"Atom at %.16e %f %f on wrong CPU.Should go to %d. x=%d,min.x=%d,max.x=%d,cpul=%d,cpur=%d,cellx:%d\n",
		  xx,yy,zz,to_cpu,x,min.x,max.x,my_coord.x-1,my_coord.x+1,cellx);
	  error(errbuf);
//	  error("atom on wrong CPU");
        }
//ENDOF MYMOD
#else
	to = PTR_VV(cell_array,cellc,cell_dim);
