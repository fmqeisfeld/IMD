...
//MY MOD: peak intensity
    else if(strcasecmp(token,"peak_intensity")==0){
        getparam("peak_intensity",&peak_intensity,PARAM_REAL,1,1);
    }	
    else if(strcasecmp(token,"peak_intensity2")==0){
        getparam("peak_intensity2",&peak_intensity2,PARAM_REAL,1,1);
    }
//MY MOD:lambda
    else if(strcasecmp(token,"lambda")==0){
	getparam("lambda",&lambda,PARAM_REAL,1,1);
    } 
    else if(strcasecmp(token,"lambda2")==0){
        getparam("lambda2",&lambda2,PARAM_REAL,1,1);
    }
#endif
#ifdef PDECAY
...
#endif
//MY MOD: peak_intensity und peak_intensity2 statt sigma_e
...
/////////MY MOD
  ...
  MPI_Bcast( &peak_intesnity, 1, MPI_REAL,0, MPI_COMM_WORLD);
  MPI_Bcast( &peak_intensity2,1, MPI_REAL,0, MPI_COMM_WORLD);
  MPI_Bcast( &lambda,	      1, MPI_REAL,0, MPI_COMM_WORLD);
  MPI_Bcast( &lambda2,	      1, MPI_REAL,0, MPI_COMM_WORLD); 
  ...
  * MY MOD   *
  ************/
  read_arr("K12.dat",&K12,&K12_rows);
  read_arr("alu_eps_bb_real.dat",&eps_bb_real_data,&eps_bb_rows);
  read_arr("alu_eps_bb_imag.dat",&eps_bb_imag_data,&eps_bb_rows); 
...
* read array function   *
*************************/
int read_arr(char* infilename,double ***arr,int *rowsout)
{
  ...
  *arr = malloc(sizeof(**arr) * (*rowsout));
  buf= malloc(sizeof(double)*2*(*rowsout));
  ...
  MPI_Bcast(buf,(*rowsout)*2,MPI_DOUBLE,0,MPI_COMM_WORLD);
...
/****************************
* MY MOD: lookup from table *
*****************************/
double lookup_table(double ***arr,int rows,double T)
{
   double delta,tmp;
   int key,i;
   delta=1e9;
