...
#ifdef MPIIO
  if(parallel_output==3 || parallel_output==4) //ansonsten das alte schema
  {
...
//MY MOD:MPI_ANY_TAG -> OUTBUF_TAG:=> Das bringt nix, irgendwo wartet ein proc auf MPI_Send or Recv...
      ...
      {
	printf("steps:%d,cpu:%d,warning:messages mixed up\n",steps,myid);
        //error("messages mixed up (imd_io.c)");
      }
...
//MYMOD local order param
#ifdef LOD
  MPI_Bcast( &(info->n_lod),   1, MPI_INT,  0, MPI_COMM_WORLD);
#endif
#ifdef VISCOUS
