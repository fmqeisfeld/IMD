...
// ********************************************
// *        PARAMS FOR TTM AND FDTD
// ********************************************

#ifdef TTM
    //MY MOD: fd_g wird dyn. berechnet
    ...
    else if(strcasecmp(token,"fd_min_atoms")==0){
...
#ifdef FDTD
else if(strcasecmp(token,"Sc")==0){
        getparam("Sc",&Sc,PARAM_REAL,1,1);
    }
else if (strcasecmp(token, "srcw")==0){
      getparam("srcw", &srcw, PARAM_REAL, 1,1);
    }
else if (strcasecmp(token, "srcx")==0){
      getparam("srcx", &srcx, PARAM_REAL, 1,1);
    }
else if (strcasecmp(token, "pml")==0){
      getparam("bw", &bw, PARAM_INT, 1,1);
    }
#endif
else if (strcasecmp(token, "lambda")==0){
      getparam("lambda", &lambda, PARAM_REAL, 1,1);
    }

#endif //FDTD LASER und TMM
//MY MOD: WIDERANGE TTM
    else if(strcasecmp(token,"atomic_weight")==0){
        getparam("atomic_weight",&atomic_weight,PARAM_REAL,1,1);
    }
    else if(strcasecmp(token,"atomic_charge")==0){
        getparam("atomic_charge",&atomic_charge,PARAM_REAL,1,1);
    }

#ifdef DIRICHLET
    else if(strcasecmp(token,"dirichlet_surfx")==0){
        getparam("dirichlet_surfx",&dirichlet_surfx,PARAM_REAL,1,1);
    }
#endif //DIRICHLET
#endif //TTM

//MY MOD: Non reflecting boundary
#ifdef NRB
else if(strcasecmp(token,"nrb_alat")==0){
        getparam("nrb_alat",&nrb_alat,PARAM_REAL,1,1);
    }
else if(strcasecmp(token,"nrb_eps")==0){
        getparam("nrb_eps",&nrb_eps,PARAM_REAL,1,1);
    }
else if(strcasecmp(token,"nrb_k")==0){
        getparam("nrb_k",&nrbk,PARAM_REAL,1,1);
    }
else if(strcasecmp(token,"nrb_overwrite")==0){
        getparam("nrb_overwrite",&nrb_overwrite,PARAM_INT,1,1);
    }
else if(strcasecmp(token,"nrb_infile")==0){
        getparam("nrb_infile",nrb_input_file,PARAM_STR,1,255);
	nrb_readfile=1;
    }
#endif


//MYMOD: LOCAL ORDER PARAMETER
#ifdef LOD //lattice spacing
    else if(strcasecmp(token,"alat")==0){
        getparam("alat",&alat,PARAM_REAL,1,1);
    }
    else if(strcasecmp(token,"lodnorm")==0){
        getparam("lodnorm",&lodnorm,PARAM_REAL,1,1);
    }
#endif

// ***********************************
// * MY MOD: shiftx_front,shiftx_rear
// ************************************
else if (strcasecmp(token, "shiftx_front")==0){
      getparam("shiftx_front", &shiftx_front, PARAM_REAL, 1,1);
    }
else if (strcasecmp(token, "shiftx_rear")==0){
      getparam("shiftx_rear", &shiftx_rear, PARAM_REAL, 1,1);
    }
else if (strcasecmp(token, "shifty_front")==0){
      getparam("shifty_front", &shifty_front, PARAM_REAL, 1,1);
    }
else if (strcasecmp(token, "shifty_rear")==0){
      getparam("shifty_rear", &shifty_rear, PARAM_REAL, 1,1);
    }
else if (strcasecmp(token, "shiftz_front")==0){
      getparam("shiftz_front", &shiftz_front, PARAM_REAL, 1,1);
    }
else if (strcasecmp(token, "shiftz_rear")==0){
      getparam("shiftz_rear", &shiftz_rear, PARAM_REAL, 1,1);
    }
...
/********************************************************************************************
* MY MOD: When restarting TTM-Simulation, user has to provide ramp_start and ramp_end       *
*         Otherwise the accelaration of the atoms in laser_rescale_ttm might be	            *
*         very different because the sample dimension changes				    *
*CAUTION: No error-handling managed yet							    *
*********************************************************************************************/
else if(strcasecmp(token,"ramp_start")==0){
	getparam("ramp_start",&ramp_start,PARAM_REAL,1,1);        
}
else if(strcasecmp(token,"ramp_end")==0){
        getparam("ramp_end",&ramp_end,PARAM_REAL,1,1);
}
...
// ***********************************************
//MY MOD: Hot fix fuer pdecay fuer +y und -y bnd
else if(strcasecmp(token,"ramp_y0max")==0){
        getparam("ramp_y0max",&ramp_y0max,PARAM_REAL,1,1);
}
else if(strcasecmp(token,"ramp_y0min")==0){
        getparam("ramp_y0min",&ramp_y0min,PARAM_REAL,1,1);
}
else if(strcasecmp(token,"ramp_y1max")==0){
        getparam("ramp_y1max",&ramp_y1max,PARAM_REAL,1,1);
}
else if(strcasecmp(token,"ramp_y1min")==0){
        getparam("ramp_y1min",&ramp_y1min,PARAM_REAL,1,1);
}
else if(strcasecmp(token,"pdecay_surfx")==0){
        getparam("pdecay_surfx",&pdecay_surfx,PARAM_REAL,1,1);
}
...
************************/
#ifdef FILTER
    else if (strcasecmp(token, "filter_int")==0){
      getparam("filter_int", &filter_int, PARAM_INT, 1, 1);
    }

    else if (strcasecmp(token, "filter_min_x")==0){
      /* atom deletion filter left cutoff value  */
      getparam("filter_min_x", &filter_min_x, PARAM_REAL, 1, 1);
    }
    else if (strcasecmp(token, "filter_max_x")==0){
      /* atom deletion filter left cutoff value  */
      getparam("filter_max_x", &filter_max_x, PARAM_REAL, 1, 1);
    }

//MYMOD:Brauche ich bei FDTD!
    else if (strcasecmp(token, "filter_min_y")==0){
      /* atom deletion filter left cutoff value  */
      getparam("filter_min_y", &filter_min_y, PARAM_REAL, 1, 1);
    }
    else if (strcasecmp(token, "filter_max_y")==0){
      /* atom deletion filter left cutoff value  */
      getparam("filter_max_y", &filter_max_y, PARAM_REAL, 1, 1);
    }

    else if (strcasecmp(token, "filter_min_z")==0){
      /* atom deletion filter left cutoff value  */
      getparam("filter_min_z", &filter_min_z, PARAM_REAL, 1, 1);
    }
    else if (strcasecmp(token, "filter_max_z")==0){
      /* atom deletion filter left cutoff value  */
      getparam("filter_max_z", &filter_max_z, PARAM_REAL, 1, 1);
    }
      ...
      // MY MOD: in imd_laser.c wird laser_sigma_w0=1/(laser_sigma_w0^2),
      //         warum auch immer...Deswegen speicher ich den beamradius
      //         zusätzlich in einer eigenen Variablen, da ich nachher
      //         zur Berechnung der Laserintensität die effektive,
      //         bestrahle flaeche brauche
      //beam_radius=laser_sigma_w0;
    }
...
// MY MOD:
  ...
  if(I0==0)
    error("You must specify peak intensity I0 in W/m^2");
  if(lambda==0)
    error("You must specify lambda in meters");
#endif //FDTD und TMM
#if defined(FDTD) || defined(LASER) || defined(TMM)
  if(fd_n_timesteps==0)
    error("You must specify fd_n_timesteps");
#endif //FDTD und LASER und TMM
  if(atomic_weight==0)
...
//MY MOD: Hotfix fuer pdecay für +/-y
  MPI_Bcast( &ramp_y0max,     1, REAL,  0, MPI_COMM_WORLD);
  MPI_Bcast( &ramp_y1max,     1, REAL,  0, MPI_COMM_WORLD);
  MPI_Bcast( &ramp_y0min,     1, REAL,  0, MPI_COMM_WORLD);
  MPI_Bcast( &ramp_y1min,     1, REAL,  0, MPI_COMM_WORLD);
  MPI_Bcast( &pdecay_surfx,   1, REAL,  0, MPI_COMM_WORLD);
#endif
//MY MOD: Non-refl. bnd-conds.
#ifdef NRB
  MPI_Bcast( &nrb_alat,      1, REAL,  0, MPI_COMM_WORLD);
  MPI_Bcast( &nrb_eps,       1, REAL,  0, MPI_COMM_WORLD);
  MPI_Bcast( &nrbk,          1, REAL,  0, MPI_COMM_WORLD);
  MPI_Bcast( &nrb_overwrite, 1, MPI_INT,  0, MPI_COMM_WORLD);
//  MPI_Bcast( &nrb_input_file,255, MPI_CHAR,  0, MPI_COMM_WORLD); //nicht noetig, da sowieso nur proc 0 einliest!
  MPI_Bcast( &nrb_readfile,   1, MPI_INT, 0, MPI_COMM_WORLD);
#endif
//MY MOD shiftx_front, shiftx_rear
MPI_Bcast (&shiftx_front, 1, REAL, 0, MPI_COMM_WORLD);
MPI_Bcast (&shiftx_rear,1,REAL,0,MPI_COMM_WORLD);

MPI_Bcast (&shifty_front, 1, REAL, 0, MPI_COMM_WORLD);
MPI_Bcast (&shifty_rear,1,REAL,0,MPI_COMM_WORLD);

MPI_Bcast (&shiftz_front, 1, REAL, 0, MPI_COMM_WORLD);
MPI_Bcast (&shiftz_rear,1,REAL,0,MPI_COMM_WORLD);
//MY MOD: filter auch in y-dir
#ifdef FILTER
MPI_Bcast( &filter_min_x,           1, REAL,    0, MPI_COMM_WORLD);
MPI_Bcast( &filter_max_x,           1, REAL,    0, MPI_COMM_WORLD);
MPI_Bcast( &filter_min_y,           1, REAL,    0, MPI_COMM_WORLD);
MPI_Bcast( &filter_max_y,           1, REAL,    0, MPI_COMM_WORLD);
MPI_Bcast( &filter_min_z,           1, REAL,    0, MPI_COMM_WORLD);
MPI_Bcast( &filter_max_z,           1, REAL,    0, MPI_COMM_WORLD);
MPI_Bcast( &filter_int,             1, MPI_INT, 0, MPI_COMM_WORLD);
#endif
...
/////////MY MOD
  MPI_Bcast( &fd_min_atoms,   1, MPI_INT, 0, MPI_COMM_WORLD);
#if defined(FDTD) || defined(LASER) || defined(TMM)
  MPI_Bcast( &lambda,	      1, REAL,0, MPI_COMM_WORLD);
#endif

  MPI_Bcast( &atomic_weight,  1, REAL,0, MPI_COMM_WORLD);
  MPI_Bcast( &atomic_charge,  1, REAL,0, MPI_COMM_WORLD);
#if defined(FDTD) || defined(LASER) || defined(TMM)
  MPI_Bcast( &I0,             1, REAL,0, MPI_COMM_WORLD);
  MPI_Bcast( &laser_sigma_t,    1, REAL,  0, MPI_COMM_WORLD);
  MPI_Bcast( &laser_t_0,        1, REAL,  0, MPI_COMM_WORLD);
  ...
  MPI_Bcast( &srcx,             1, REAL,0, MPI_COMM_WORLD);
  MPI_Bcast( &bw,               1, MPI_INT,0, MPI_COMM_WORLD);
  MPI_Bcast( &Sc,	        1, REAL,0,MPI_COMM_WORLD);
#endif //FDTD
#endif /* TTM */
//MYMOD: LOCAL ORDER PARAM
#ifdef LOD
 MPI_Bcast( &alat,    1, REAL,0, MPI_COMM_WORLD);
 MPI_Bcast( &lodnorm,    1, REAL,0, MPI_COMM_WORLD);
#endif
#ifdef DIRICHLET
 MPI_Bcast(&dirichlet_surfx,1,REAL,0,MPI_COMM_WORLD);
#endif
