...
void fix_cells(void)
{
  ...
  //printf("proc:%d,steps:%d start empty_mpi_buffers (fix cells)\n",myid,steps);
#ifdef MPI
  empty_mpi_buffers();
#endif
  //printf("proc:%d,steps:%d done empty_mpi_buffers (fix cells)\n",myid,steps);


  /* apply periodic boundary conditions */
  //printf("proc:%d,steps:%d start do_boundaries (fix cells)\n",myid,steps);
  do_boundaries();
  //printf("proc:%d,steps:%d done do_boundariess (fix cells)\n",myid,steps);


  //printf("proc:%d,steps:%d start loop(fix cells)\n",myid,steps);
  ...
  for (i=cellmin.x; i < cellmax.x; ++i)
	...
	while( l<p->n ) {
	  ...
	  /* check whether the atom has to be filtered out */
//        if(steps>0 && 0 == steps % filter_int)
//#ifdef FILTERX  //immer automatisch mit FILTER aktiv
	  if (ORT(p,l,X) < filter_min_x || ORT(p,l,X) > filter_max_x
//#ifdef FILTERY
	      || ORT(p,l,Y)<filter_min_y || ORT(p,l,Y)>filter_max_y
//#endif  //FILTERY

//#ifdef FILTERZ
	      || ORT(p,l,Z)<filter_min_z || ORT(p,l,Z)>filter_max_z
//#endif  //FILTERZ
//#endif  //FILTERX 
	  )
	  {
...
#ifdef NBL
	      have_valid_nbl = 0;  //mui importante
#endif

/*
#ifdef TTM
	      printf("proc:%d, steps:%d,Atom %d at %f %f %f removed,temp:%f,md-temp:%f,natoms:%d\n",
		myid,steps,l,ORT(p,l,X), ORT(p,l,Y), ORT(p,l,Z),l1[fd_i][fd_j][fd_k].temp,l1[fd_i][fd_j][fd_k].md_temp,l1[fd_i][fd_j][fd_k].natoms );
#endif
*/
            if (buf != NULL) {
...
#else
              printf("ERROR on cpu:%d at step:%d: Atom %d jumped multiple CPUs. px:%f,py:%f,pz:%f,fx:%f,fy:%f,fz:%f,x:%f,y:%f,z:%f,rx:%f,ry:%f,rz:%f\n",
