...
double get_surface()
{
  ...
  /**************************************************************************
  * MY MOD: Sollte es sich um eine fortgesetzte Simulation handeln, so      *
  *         werden ramp_start und ramp_end aus der Param-Datei übernommen   *
  ***************************************************************************/
  if(imdrestart==0) 
  {  
  ...
  //xsurf= (double)(leftside+0.5)*deltax;
...
#ifdef LASERYZ
//MYMOD
  laser_spot_area=M_PI*pow(laser_sigma_w0*1e-10,2.0); //in m weil Fabs in J/m^2
//ENDOF MYMOD
...
//MYMOD ich möchte das selber angeben!
#ifndef LASERYZ 
...
//ENDOF MYMOD
  ...
  * MY MOD     *
  **************/
//  double cross_sect=box_y.y*box_z.z;
//#ifdef LAYSERYZ
//  cross_sect=2*M_PI*beam_radius*beam_radius;
//#endif
  //peak_power=laser_sigma_e*cross_sect/(laser_sigma_t*2.35); // eV/Angstrom^2 * Angstrom^2/IMD-time
  //peak_intensity=peak_power/(cross_sect);                   // eV/Angstrom^2/IMD-time
  //peak_intensity=laser_sigma_e/(laser_sigma_t*2.35*4);
  //peak_power2=laser_sigma_e1*cross_sect/(laser_sigma_t1*2.35);
  //peak_intensity2=peak_power2/(cross_sect);
  //ACHTUNG: Die intensitäten müssen noch durch global_fd_dim.y*global_fd_dim.z geteilt werden,
  //         sonst bekommt JEDE FD-Zelle die Volle Intensität ab, die eigentlich für die
  //	     komplette Oberfläche bestimmt ist
  //	     => Steht erstmal am Ende von ttm_init (denn erst dort wird fd_global.y und z 
  //	     initialisiert
}
...
void laser_rescale_ttm()
{
  ...
  gauss_time_squared = timestep * steps - laser_t_0;
  gauss_time_squared *= gauss_time_squared;
  exp_gauss_time_etc = exp(-gauss_time_squared/laser_sigma_t_squared/2.0)* laser_p_peak;

//MYMOD: Brauch ich erstmal net
/*
  if (laser_t_1>0) {
    gauss_time_squared1 = timestep * steps - laser_t_1;
    gauss_time_squared1 *= gauss_time_squared1;
...
//ENDOF MYMOD
  ...
  double x,y,z;
  //MYMOD Laser active ?
  if(exp_gauss_time_etc<laser_p_peak*0.001)
    laser_active=false;
  else
    laser_active=true;
  //ENDOF MYMOD
  ...
  for(i=1; i<local_fd_dim.x-1; ++i)
  {
    for(j=1; j<local_fd_dim.y-1; ++j)
    {
      ...
      {
// ACHTUNG: Per default ist tem-mod: 0,0,0 == laser_intensity_profile_laguerre_00
if(l1[i][j][k].natoms>=fd_min_atoms)
{
	depth = ttm_calc_depth(i,j,k); //standard,bei 1d simul 
#ifdef LASERYZ 
        x = fd_h.x/2+fd_h.x * ((i-1) + my_coord.x*(local_fd_dim.x-2)); //l1[i][j][k].x_com;
        y = fd_h.y/2+fd_h.y * ((j-1) + my_coord.y*(local_fd_dim.y-2));//l1[i][j][k].y_com;
        z = fd_h.z/2+fd_h.z * ((k-1) + my_coord.z*(local_fd_dim.z-2));//l1[i][k][k].z_com;

	depth=x-laser_offset;

        l2[i][j][k].source=l1[i][j][k].source = exp( -laser_mu*depth ) * exp_gauss_time_etc * laser_intensity_profile(x,y,z);

if(laser_intensity_profile(x,y,z)>1.0)
  printf("myid:%d,src:%.4e,prof:%f,x:%f,y:%f,z:%f\n",myid,l1[i][j][k].source,laser_intensity_profile(x,y,z),x,y,z);

//printf("src:%e,i:%d,j:%d,k:%d,gausstime:%.4e,intprof:%.4e,depth:%.4e,laser_mu:%.4e,ofs:%.4e\n",l2[i][j][k].source,
//       i,j,k,exp_gauss_time_etc,laser_intensity_profile(x,y,z),depth,laser_mu,laser_offset);
#else

        l1[i][j][k].source = l1[i][j][k].source=exp(-laser_mu*depth) * exp_gauss_time_etc;

//  exp_gauss_time_etc = exp(-gauss_time_squared/laser_sigma_t_squared/2.0)* laser_p_peak;



//printf("myid:%d,src:%.4e,depth:%f,gausstimeetc:%.4e,sigmatsq:%.4e,ppeak:%.4e,gauss_sq:%.4e\n",
//	myid,l1[i][j][k].source,depth,exp_gauss_time_etc,
//	laser_sigma_t_squared,laser_p_peak,gauss_time_squared);
...
#ifdef PDECAY  			//direkt in force-loop um zusätzliche Schleife zu vermeiden
  for (k=0; k<NCELLS; k++) {
    cell *p;
    p = CELLPTR(k);
    int i;
	...
	if( ORT(p,i,X) > ramp_start )
	  {
		...
		if(steps==-1)
	        {
		  printf("%f %f \n",  ORT(p,i,X), a * ( ORT(p,i,X) - ramp_start ) * ( ORT(p,i,X) - ramp_start ) );
	          printf("ramp_start:%f,ramp_end:%f\n",ramp_start,ramp_end);
                }
		break;
	      }
	      break;
	    /*************************************************************
            * MY MOD: damped atoms should have at least a Temp of 300K	 *
            **************************************************************/  
            case 4:
              {
                double a = 1.0 / ( ramp_end - ramp_start);
	        ...
	        int fd_i=p->fd_cell_idx.x;
                ...
                double delta_T=ABS(l1[fd_i][fd_j][fd_k].md_temp-0.0258);	       
                double delta_imp=SQRT(3*MASSE(p,i)*delta_T-IMPULS(p,i,Y)*IMPULS(p,i,Y)-IMPULS(p,i,Z)*IMPULS(p,i,Z));
	        delta_imp*=(1/(ramp_end-ramp_start))*(1/(ramp_end-ramp_start))*(ORT(p,i,X)-ramp_start)*(ORT(p,i,X)-ramp_start)*xipdecay;
		IMPULS(p,i,X)-=delta_imp;
                if(steps==-1)
                {
                  printf("%f %f \n",  ORT(p,i,X), a * ( ORT(p,i,X) - ramp_start ) * ( ORT(p,i,X) - ramp_start ) );
                  printf("ramp_start:%f,ramp_end:%f\n",ramp_start,ramp_end);
