...
LIBS		+= ./nn_interpol/libnn.a -lm  #Fuer EOS interpolation
MPI_FLAGS	+= #-DMPI
OMP_FLAGS	+= #-DOMP
...
ifeq (icc,${IMDSYS})
  ...
  BIN_DIR       = ${HOME}/bin/
  OPT_FLAGS     += -g #-O2 -Wno-deprecated-declarations -funroll-loops -m64 #-vec-report 2 -ffast-math #-fopt-info-vec #-O0 -m64 -Wno-unused
  OMPI_FLAGS    += #-openmp
  DEBUG_FLAGS   += -g
  PROF_FLAGS    += #-g3 -pg
...
ifeq (gcc,${IMDSYS})
  ...
  MPICH_CC      = gcc-8
  MPICH_CLINKER = gcc-8
  OMPI_MPICC    = gcc-8 #mpicc
  BIN_DIR       = ${HOME}/bin/
  OPT_FLAGS     += -O2 -Wno-deprecated-declarations -funroll-loops -march=corei7-avx -mtune=corei7-avx -mavx2 -ftree-vectorize -m64 -ffast-math\
