...
{
//  printf("\n\n\n proc:%d,steps:%d move_atom_mini entered. from-n>n:%d\n",myid,steps,from->n);
...
//  printf("proc:%d,steps:%d move_atom_mini done\n",myid,steps);
...
//MYMOD
#ifdef TTM
  to->fdi[i]=from->fdi[j];  //global fd-cell indices for advection step!
  to->fdj[i]=from->fdj[j];
  to->fdk[i]=from->fdk[j];
#endif
#ifdef NRB
  int r;
  to->isnrbbnd[i]=from->isnrbbnd[j];
  to->isnrbneigh[i]=from->isnrbneigh[j];

  for(r=0;r<12;r++)
  {
      to->nrbid[i*12+r]=from->nrbid[j*12+r];
  }
...
#endif
#ifdef FILTER
  to->filterme[i]=from->filterme[j];
  to->keepme[i]=from->keepme[j];
  to->delme[i]=from->delme[j];
  to->filterflag[i]=from->filterflag[j];
#endif 
//ENDOF MYMOD
...
*/
  ...
  memalloc( &p->impuls_alt,   n*SDIM, sizeof(real), al, ncopy*SDIM, 0, "impuls_alt" ); //muss nicht kommuniziert werden!
#endif

#ifdef FILTER
  memalloc( &p->filterme,n,sizeof(integer),al,ncopy,1,"filterme");
  memalloc( &p->keepme,n,sizeof(real),al,ncopy,1,"keepme");
  memalloc( &p->delme,n,sizeof(integer),al,ncopy,1,"delme");
  memalloc( &p->filterflag,n,sizeof(integer),al,ncopy,1,"filterflag");
#endif
