...
void write_atoms_config_mpiio(void)
{
  ...
  if (myid==0){
   ...
   MPI_File_write_at(fh, 87, "Number, Mass, Position X,Y,Z, Velocity X,Y,Z", 44, 
                     MPI_CHAR, &iostatus);
  };
...
if(myid==0)
{
  ...
  MPI_Bcast( &readatoms, 1,MPI_LONG, 0,MPI_COMM_WORLD);
  MPI_Bcast( &readobservables,1,MPI_SHORT,0,MPI_COMM_WORLD);
  ...
  // NOW READ ATOMS
  ////////////////////////7
  long atoms_per_cpu=(long) ((double) readatoms)/((double)num_cpus);
  long atoms_per_cpu_extra; //nuer fuer letzten proc von bedeutung
  if(myid==num_cpus-1)
        atoms_per_cpu_extra=atoms_per_cpu+(((long) readatoms)-atoms_per_cpu*num_cpus);
...
*/
  ...
  myofs=(long) (disp+ myid*atoms_per_cpu*byte*obs);

  MPI_File_set_view(fh, myofs, MPI_DOUBLE, MPI_DOUBLE, DATAREP,
                    MPI_INFO_NULL);
  if(myid!=num_cpus-1)
    MPI_File_read_all(fh,atomsbuffer,(long) atoms_per_cpu*obs,MPI_DOUBLE,&iostatus);
  else //letzter proc muss unter umstaenden mehr lesen
  {
    MPI_File_read_all(fh,atomsbuffer,(long) atoms_per_cpu_extra*obs,MPI_DOUBLE,&iostatus);
  ...
  for(k=0;k<num_cpus;k++)
  {
	...
	myofs=(long) i*obs;
	input->n=1;
	NUMMER(input,0)=atomsbuffer[myofs]; //nr;
	MASSE(input,0)=atomsbuffer[myofs+1];
	ORT(input,0,X)=atomsbuffer[myofs+2]+shiftx_front;    
	ORT(input,0,Y)=atomsbuffer[myofs+3]+shifty_front;
	ORT(input,0,Z)=atomsbuffer[myofs+4]+shiftz_front;
	IMPULS(input,0,X)=atomsbuffer[myofs+5]*MASSE(input,0);
	IMPULS(input,0,Y)=atomsbuffer[myofs+6]*MASSE(input,0);
	IMPULS(input,0,Z)=atomsbuffer[myofs+7]*MASSE(input,0);
...
// ******************** WRITE ATOMS VERSION FUER COPPLESTONES PLASMA-KRAM ******************
/*
#define OBSERVABLES2 8 // x,y,z,vx,vy,vz,Te,Z
void write_atoms_config_mpiio(void)
{
  if(myid==0)
    printf("MPI-OUTPUT COPPLESTONE VERSION!!!\n");
  int i, k, n;
  long mpi_atoms=0;
  char *atomsbuffer;
  double mpidata[OBSERVABLES2];
  long startidx=0;
  int observables=OBSERVABLES2;
  int byte=8;

  char *mpioutfilename, *filename;
  int atombuffersize, atombufferpos=0;
  MPI_Pack_size (OBSERVABLES2, MPI_DOUBLE, MPI_COMM_WORLD, &atombuffersize);
  MPI_File fh;
  MPI_Offset disp=1024, myoffset;
  MPI_Status iostatus;
  ...
  for (k=0; k<NCELLS; k++) {
    ...
    double cTe=l1[fd_i][fd_j][fd_k].temp;
    double cZ= l1[fd_i][fd_j][fd_k].Z;
	...
	mpidata[n++] = (IMPULS(p,i,Z) / MASSE(p,i));
        mpidata[n++] = cTe;
        mpidata[n++] = cZ; 
	MPI_Pack (mpidata, OBSERVABLES2, MPI_DOUBLE, atomsbuffer, 
                  observables*byte*mpi_atoms, &atombufferpos, MPI_COMM_WORLD);
    }
  }
  ...
  if (myid==0){
   ...
   MPI_File_write_at(fh, 87, "X,Y,Z,VX,VY,VZ,T,Z", 18, 
                     MPI_CHAR, &iostatus);
  };

  myoffset=disp+startidx*byte*OBSERVABLES2;
